...
const spacetimedb = schema({
	...
	rate_limit: table({ name: "rate_limit" }, {
		key: t.string().primaryKey(),
		last_action: t.u64()
	})
});
spacetimedb.reducer({ name: "create_session" }, {
	session_id: t.string(),
	speaker_name: t.string(),
	topic: t.string()
...
spacetimedb.reducer({ name: "submit_signal" }, {
	id: t.string(),
	session_id: t.string(),
	signal_type: t.string(),
	audience_id: t.string(),
...
spacetimedb.reducer({ name: "log_intervention" }, {
	id: t.string(),
	session_id: t.string(),
	message: t.string(),
	suggestion: t.string(),
...
spacetimedb.reducer({ name: "submit_caption" }, {
	id: t.string(),
	session_id: t.string(),
	text: t.string()
...
spacetimedb.reducer({ name: "acknowledge_intervention" }, { id: t.string() }, (ctx, { id }) => {
	const row = ctx.db.intervention.id.find(id);
	if (row) ctx.db.intervention.delete(row);
	if (row) ctx.db.intervention.insert({
		...row,
		acknowledged: true
	});
});
spacetimedb.reducer({ name: "submit_question" }, {
	id: t.string(),
	session_id: t.string(),
	text: t.string(),
	audience_id: t.string()
...
spacetimedb.reducer({ name: "upvote_question" }, {
	question_id: t.string(),
	audience_id: t.string()
}, (ctx, { question_id, audience_id }) => {
	if ([...ctx.db.question_upvote.iter()].find((r) => r.question_id === question_id && r.audience_id === audience_id)) return;
...
spacetimedb.reducer({ name: "dismiss_question" }, { question_id: t.string() }, (ctx, { question_id }) => {
	const q = ctx.db.question.id.find(question_id);
	if (q) {
		ctx.db.question.delete(q);
		ctx.db.question.insert({
...
spacetimedb.reducer({ name: "answer_question" }, { question_id: t.string() }, (ctx, { question_id }) => {
	const q = ctx.db.question.id.find(question_id);
	if (q) {
		ctx.db.question.delete(q);
		ctx.db.question.insert({
...
spacetimedb.reducer({ name: "submit_mood" }, {
	id: t.string(),
	session_id: t.string(),
	word: t.string(),
	audience_id: t.string(),
...
spacetimedb.reducer({ name: "create_poll" }, {
	id: t.string(),
	session_id: t.string(),
	question: t.string(),
	options: t.string()
...
spacetimedb.reducer({ name: "vote_poll" }, {
	poll_id: t.string(),
	audience_id: t.string(),
	option: t.string()
}, (ctx, { poll_id, audience_id, option }) => {
	if ([...ctx.db.poll_vote.iter()].find((r) => r.poll_id === poll_id && r.audience_id === audience_id)) return;
	ctx.db.poll_vote.insert({
		id: `${poll_id}:${audience_id}`,
		poll_id,
		audience_id,
		option
	});
...
spacetimedb.reducer({ name: "close_poll" }, { poll_id: t.string() }, (ctx, { poll_id }) => {
	const poll = ctx.db.poll.id.find(poll_id);
	if (poll) {
		ctx.db.poll.delete(poll);
		ctx.db.poll.insert({
...
spacetimedb.reducer({ name: "end_session" }, { session_id: t.string() }, (ctx, { session_id }) => {
